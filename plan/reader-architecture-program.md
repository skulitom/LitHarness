...
| **Persistent reader simulation** — attention, expectation, confusion and voluntary stopping over a trajectory | reading as behaviour rather than as one isolated verdict | `bcr.v0` and `fcr.v0` (budgeted continuation; the costed feed with skim and abandonment). **Seated and moved, 2026-09-04**: `fcr.v0` pointed at a whole-book paragraph shuffle allocated fewer full reads to the shuffled target than to the intact one and than to a whitespace placebo, replicated with the permutation redrawn (stage-0 §230, two registered arms of 180 sessions each). **The first family here to move with a story-level manipulation and hold under replication** — and one faculty on the loudest available stimulus, satisfying one of the ten fields `domain/editorial.QualificationEvidence` requires. What it would take, what it would feed and what would withdraw it are in [`reader-architecture-proposal.md`](reader-architecture-proposal.md); the gradient validation below is untouched by any of it |
