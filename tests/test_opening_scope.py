"""Opening-only house guidance follows accepted reading history, never arc ordinals."""
...
def test_continuing_house_block_changes_only_the_opening_requirement():
    complete = house.with_house_rules("Task.")
    assert complete == f"Task.\n\n{house.HOUSE_RULES}"
    assert complete == house.with_house_rules("Task.", opening=True)
    assert house.with_house_rules("Task.", opening=False) == complete.replace(
        house.OPENING_OFFER + "\n", "", 1,
    )
    assert house.with_house_rules("", opening=False) == house.HOUSE_RULES.replace(
        house.OPENING_OFFER + "\n", "", 1,
    )
...
def test_later_arc_keeps_continuation_scope_when_prior_prose_is_evicted(tmp_path):
    ...
    with SqliteStore.open(tmp_path / "later-arc.db") as store:
        ...
        assert house.ACCUMULATION in selected.payload["system"]
...
def test_an_explicit_author_lock_is_not_removed_by_house_scope(tmp_path):
    from litharness.adapters.sqlite_store import SqliteStore

    locked = lc.PlanItem(
        logical_id="author-opening-offer", kind=lc.PlanKind.CONSTRAINT,
        text=house.OPENING_OFFER, locked=True, authority=lc.PlanAuthority.INTENDED,
    )
    with SqliteStore.open(tmp_path / "locked.db") as store:
        ...
        assert house.OPENING_OFFER not in guidance
        assert house.OPENING_OFFER in author_locks
