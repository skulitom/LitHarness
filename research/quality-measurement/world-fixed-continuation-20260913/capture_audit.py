...
def audit():
    ...
    for index, book in enumerate(progress["books"], 1):
        ...
        checkpoints = []
        earlier_hashes = {}
        for label in ("empty", "seeded", "checkpoint-1", "checkpoint-2", "checkpoint-3", "final"):
            ...
            controls[f"{book['id']}_{label}_earlier_chapters_preserved"] = all(
                chapter_hashes.get(name) == digest for name, digest in earlier_hashes.items()
            )
            earlier_hashes = chapter_hashes
            checkpoints.append({
        ...
        controls[f"{book['id']}_final_exports_match"] = earlier_hashes == {
            path.name: sha(path)
            for path in sorted((directory / "library").glob("*/chapters/Chapter*.txt"))
        }
        books.append({
    ...
    assigned = registration["chapters_by_book"]
    completed = len(books) == len(assigned) and all(
        book["status"] == "completed" and book["chapters"] == expected
        and len(book["checkpoints"][-1]["chapter_hashes"]) == expected
        for book, expected in zip(books, assigned, strict=True)
    )
    save(HERE / "evidence.json", {
        "schema": "litharness.world-fixed-continuation-evidence.v1", "status": "observed",
