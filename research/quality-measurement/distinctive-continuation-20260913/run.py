...
ROOT = HERE.parents[2]
RUN = ROOT / "runs/distinctive-continuation-20260913"
REVISION = "HEAD"
OWNER = "distinctive-continuation-20260913: root task;"
MAX_CALLS = 60
MAX_TOKENS = 1_800_000
BOOK_CALLS = 60
BOOK_TOKENS = 1_800_000
...
def prepare() -> None:
    ...
    manifest = {
        ...
        "version": version,
        "experiment_sha256": {name: sha(HERE / name) for name in ("run.py", "RUNBOOK.md")},
        "dependencies": sorted(
...
def run() -> None:
    ...
    def command(args: list[str], *, name: str = "command") -> tuple[int, str]:
        ...
        argv = [
            ...
            "--holder",
            "distinctive-continuation-20260913",
            "--chapter-scenes",
            "1",
            "--arc-chapters",
            "6",
    ...
    try:
        for index in range(1, 2):
            ...
            try:
                ...
                discovery = Discovery.from_invention(parent_plan())
                ...
                save(active_dir / "concept-parent.json", {
                    "path": PARENT.relative_to(ROOT).as_posix(), "sha256": PARENT_SHA,
                ...
                if book["chapters"] != 3 or len(book["chapter_hashes"]) != 3:
                    raise StopRun("Expected exactly three accepted chapter exports")
