...
ROOT = HERE.parents[2]
RUN = ROOT / "runs/world-boundary-fixes-20260913"
REVISION = "HEAD"
OWNER = "world-boundary-fixes-20260913: root task;"
MAX_CALLS = 40
MAX_TOKENS = 1_600_000
BOOK_CALLS = 25
BOOK_TOKENS = 1_000_000
...
CONTROL = RUN / "control-treatment.json"
...
def prepare() -> None:
    ...
    save(HERE / "registration.json", {
        ...
        "assigned_books": 2, "chapters_by_book": [1, 0],
    })
    ...
    try:
        for index in range(1, 3):
            ...
            try:
                ...
                checkpoint("seeded")
                for chapter in range(1, 2) if index == 1 else ():
                    for _ in range(12):
                        must(["tick"], "tick")
                        code, output = command(
