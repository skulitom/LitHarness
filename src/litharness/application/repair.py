...
def repair_job_for(
    ...
    **The predicate:** blocking AND deterministic AND span-carrying. The span requirement is
    non-negotiable: without a located span there is nothing `apply_patch` can license, and
    relaxing it would weaken the `UNLICENSED_DELETION` guarantee — the exact trade the
    planning-drafting map warns the generalization against.
    """
    span = primary_span_of(finding)
    if span is None or not (finding.blocks and finding.deterministic):
        return None
    payload: dict[str, object] = {
        "book_id": book_id,
        "branch_id": branch_id,
...
def make_repair_handler(
    ...
    *,
    ...
    def handle(job: Job, now: float) -> Sequence[Event]:
        ...
        # The mint-time license, re-checked at claim time: a blocking finding may have been
        # dismissed since the job was minted, and the repair then no longer has a license —
        # quietly completing is the honest outcome (the job was legitimate work).
