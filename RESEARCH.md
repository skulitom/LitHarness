...
| Derive anticipations from strictly earlier facts, before the incoming event is visible | Three independent prefix-only proposals, shared by two prospective drafts, compared with two fresh source-only drafts. Tests the whole guidance package, including extra information and prompt length; no results yet. | `REGISTERED` | [registration](research/quality-measurement/prose-prospective-attention/PREREG.md) |
