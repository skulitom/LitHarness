...
<!-- research-overview: ledger through §263; checked 2026-09-24 -->
...
held under replication (§230), and moved again, as far, when a seeded partial shuffle reordered
65% of paragraphs (§259), though on a reader that had drifted from §230's; every Claude reader
arm before 2026-09-22 ran where the CLI could show it the repository's git status (§258). It
satisfies, arguably, one of the ten evidence fields qualification requires. That is the whole of
what the research has bought on the central question so far. Apart from that arm, no registered
reader arm has run since 2026-09-04; the only other simulated reads of chapters on record since
...
| The one further experiment §230 licenses: the same twenty books at a seeded partial shuffle of 65% of paragraphs, six sessions per version, re-bought clean after §258. The reader read the shuffled book less, beyond a whitespace sham (+0.194 [+0.134, +0.252]), no less than at the full shuffle; but on requests identical to v2's it read the intact book +0.153 more than v2's reader, so the result stands on its own reader, not as an extension of §230 | `OBSERVED` | §259; [RESULTS.md](research/quality-measurement/cost-that-bites-milder-20260922/RESULTS.md), [PREREG.md](research/quality-measurement/cost-that-bites-milder-20260922/PREREG.md) and [AMENDMENT-1.md](research/quality-measurement/cost-that-bites-milder-20260922/AMENDMENT-1.md) |
...
| Change reasoning effort while keeping chapter inputs identical | Low drafts are shorter in both draws, but repeated explanations and display grammar problems remain. Added door testing varies within both settings. | `OBSERVED` | [registration A](research/quality-measurement/prose-reasoning-disclosure/PREREG.md#a-reasoning-effort), [results A](research/quality-measurement/prose-reasoning-disclosure/RESULTS.md#a-identical-input-requested-high-versus-low-reasoning) |
| Reveal source progressively while holding installment boundaries fixed | Both conditions produce compressed installments, repeat prices and interpretations, and avoid located unreleased-event previews. One staged draft weakens glove appearance; one ahead draft integrates the display correctly. | `OBSERVED` | [registration B](research/quality-measurement/prose-reasoning-disclosure/PREREG.md#b-progressive-disclosure-with-a-matched-installment-control), [results B](research/quality-measurement/prose-reasoning-disclosure/RESULTS.md#b-future-source-visible-versus-released-in-installments) |
| Separate binding writer facts from required reader-facing narration | Both selective drafts explicitly narrate all eleven optionally implicit units. Repeated explanations persist; shorter text does not establish a gain. Required-information omissions and new actions remain. | `OBSERVED` | [registration](research/quality-measurement/prose-narration-obligations/PREREG.md), [results](research/quality-measurement/prose-narration-obligations/RESULTS.md) |
| Actually withhold optional source units; compare a narrow intermediary with a fixed filter | The intermediary keeps two causal interpretations that the fixed reduced drafts leave unstated. Inventories fall, but new explanations persist. One selector and two draws per source condition; no qualified gain. | `OBSERVED` | [registration A](research/quality-measurement/prose-editor-boundary/PREREG.md#a-actual-source-withholding-and-an-intermediary), [results A](research/quality-measurement/prose-editor-boundary/RESULTS.md#a-actual-withholding-and-the-intermediary) |
| Compare cooperative mechanical trouble with conflicting immediate intentions | Disagreement changes the terms of an exchange; both kinds of scene still explain and repeat authority limits. Simpler ordinary-exam fixtures remove the original supernatural burden, so no original-chapter effect is established. | `OBSERVED` | [registration B](research/quality-measurement/prose-editor-boundary/PREREG.md#b-mechanical-obstacle-versus-conflicting-intentions), [results B](research/quality-measurement/prose-editor-boundary/RESULTS.md#b-cooperative-mechanics-and-conflicting-intentions) |
| Draft in Russian and render into English, with English rewrite controls | All four renderings preserve paragraph counts and explanatory structure. Terminology/spatial drift and one contextual repair are recorded; no dependable prose improvement. | `OBSERVED` | [registration C](research/quality-measurement/prose-editor-boundary/PREREG.md#c-russian-drafting-followed-by-english-rendering), [results C](research/quality-measurement/prose-editor-boundary/RESULTS.md#c-russian-to-english-and-english-to-english-chains) |
| Compare fresh subscription writers on identical full/reduced inputs | Stopped after the first Claude response reported auxiliary Haiku usage. Raw failure and offline recovery retained; three Claude calls skipped under amendment. No complete writer comparison or ranking. | `OBSERVED` | [registration D](research/quality-measurement/prose-editor-boundary/PREREG.md#d-fresh-subscription-writer-comparison), [amendment](research/quality-measurement/prose-editor-boundary/AMENDMENT.md), [results D](research/quality-measurement/prose-editor-boundary/RESULTS.md#d-writer-comparison-stopped-raw-response-retained) |
| Give existing resistance a conflicting objective under the full supernatural chapter burden | Both conflict drafts express refusal and purposeful resistance, then explain the motive again. Both aligned controls also repeat motive commentary; opening/rule exposition persists. Fixed rescue choreography limits the result; no dependable prose fix. | `OBSERVED` | [registration](research/quality-measurement/prose-conflicting-objectives/PREREG.md), [results](research/quality-measurement/prose-conflicting-objectives/RESULTS.md) |
| Add a required ending and then the detailed event sequence to a shared premise and world | All six drafts retain interpretive narration. Both premise drafts choose different rescues and stop before closure/purchases; ending drafts reach the required final conditions through alternate events. Detailed drafts retain familiar exposition. Event freedom changes the plot but supplies no dependable prose fix; load and prompt size change together. | `OBSERVED` | [registration](research/quality-measurement/prose-constraint-levels/PREREG.md), [results](research/quality-measurement/prose-constraint-levels/RESULTS.md) |
| Use a source-derived attention trajectory as background versus operative narration guidance | The one trace passed structural checks but summarized later understanding as current concerns before their listed interruptions. Temporal review stopped all four chapter calls. No prose effect was tested; retained proposal exposes missing before/after states and activation times. | `OBSERVED` | [registration](research/quality-measurement/prose-attention/PREREG.md), [results](research/quality-measurement/prose-attention/RESULTS.md) |
...
| A registered chapter-one draw on the production default path tested the restored directions end to end: draw 1 passed its concept and failed its listing (no System, no rank named), the listing prompt was fixed (§261), and draw 2 passed the concept, listing, world and chapter-one gates on the coordinator's reads; chapter one went to the operator, whose read is the test | `OBSERVED` | [research/quality-measurement/restored-directions-draw-20260922/RESULTS.md](research/quality-measurement/restored-directions-draw-20260922/RESULTS.md) |
| The operator's read of that chapter one (read 20): he would probably stop around the listing and the opening, the listing read as a list of facts rather than a hook, a rent line in the first paragraph was the old money register again, and he asked what a "surviving tread" is. Each traced to our own text: §261's listing clause, §255's "a job that covers rent" outside §174's guard, debt-named concept keys and the Architect's payment wording, and a planner brief restating a capability limit. Those causes are removed, the listing asks for a hook, and a chapter-one lane with tightened gate items and an inert register report replaces the closed draw. Instruction delivery only: no draw has tested it | — | §262; [plan/reader-read-20.md](plan/reader-read-20.md) |
...
| The comparison that decides whether the candidate roles move tier: the full-book trial's 24 scene-summary requests on gpt-6-luna against gpt-6-astra controls, and a gpt-6-sol seed screen. Luna misplaced evidence quotes (12 to 18 of 24 located, against 24) and misjudged paid promises (8 to 17 against 23), so summaries stay strong; the Sol screen saw no gross failure in two replicates, which licenses nothing | `OBSERVED` | §260; [research/quality-measurement/model-tiers-20260922/RESULTS.md](research/quality-measurement/model-tiers-20260922/RESULTS.md) |
