...
| Activate attention states after source events, carrying the previous state forward in code | Opening-only initial state, one event-anchored update proposal and four matched chapter draws registered. Evidence availability and activation order are checked; before-states cannot be regenerated with hindsight. No outputs yet. | `REGISTERED` | [registration](research/quality-measurement/prose-attention-events/PREREG.md) |
