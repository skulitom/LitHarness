# LitHarness operator guide

Use the [README](../README.md) for a first run. This guide holds the detailed workflows,
generation behavior, and inspection contracts; read the section relevant to your task.
Commands run from the repository root. `uv run litharness COMMAND --help` owns the option list.

- [Install and providers](#install)
- [Start a serial](#start-a-serial)
- [Run the production loop](#run-the-production-loop)
- [Inspect a book](#inspect-a-book)
- [Read through MCP](#read-through-mcp)
- [Direct and recover](#direct-and-recover)
- [Covers, library, and export](#covers-library-and-export)
- [Story state and time](#story-state-and-time)
- [Integrity and continuity](#integrity-and-continuity)
- [Generation details](#generation-details)
- [Simulated readers](#simulated-readers)
- [Controlled comparisons](#controlled-comparisons)

The production loop can generate and export books. **No simulated reader mechanism has yet
earned the right to certify literary quality.** Research status and supporting evidence live
in [RESEARCH.md](../RESEARCH.md) and the
[reader-architecture programme](../plan/reader-architecture-program.md).
...
The standalone clean-start generator has been retired. The `litharness` CLI is the supported
generation pipeline; the [comparison record](generation-comparison-20260908.md) preserves
...
Use `--brief` for your premise or constraints. The
...
### Writer, viewpoint, and chapter size
...
### Start from an existing premise or manuscript
...
### Scenes and recorded traces

The [MCP read guide](../.claude/skills/litharness-mcp/SKILL.md) covers the tools. Beside the verbs'
...
### Disclosure and historical plans
...
### Reading copies and release volumes
...
```powershell
$env:LITHARNESS_CONTINUITY_EVALUATOR = "C:\DEV\ContinuityEvaluation\.venv\Scripts\continuity-evaluate.exe"
```
...
These notes describe current prompts, stored artifacts, and compatibility limits.
Authoring directions and integrity checks do not establish reader appeal.

### Discovery and world invention
...
### World rules, character assertions, and status displays
...
### Scene plans and drafting context
...
### Draft provenance and optional revision
...
### Continuation and accepted history
...
### Protected rules and character knowledge
...
The bundled mechanism is experimental. Its observations cannot steer production or
certify a book's quality.
...
### Call the readership from another pipeline
...
For code changes, use [AGENTS.md](../AGENTS.md), [CONTRIBUTING.md](../CONTRIBUTING.md),
and the [system model](system-model.md).
