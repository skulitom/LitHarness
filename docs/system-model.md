...
| Status-line grants become holdings | `extraction._holdings_from_line` | a system's column binds as its `can_do` magnitude at the printed number (§236), except a priced grant printed as a quantity its price could not have bought: that binds at the purchases the line's own stock column paid for, or not at all, and the snapshot keeps the printed number (§254) | `test_priced_holdings.py::test_the_one_point_duration_purchase_binds_one_investment_and_balances_the_stock`, `test_a_grant_that_moved_on_the_status_line_records_the_holding_the_line_states` |
