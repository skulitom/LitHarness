...
| Status-line grants become holdings | `extraction._holdings_from_line` | a system's column binds as its `can_do` magnitude at the printed number (§236), except a priced grant printed as a quantity its price could not have bought: that binds at the purchases the line's own stock column paid for beyond the counts it bought as printed, or not at all, and the snapshot keeps the printed number (§254) | `test_priced_holdings.py::test_the_one_point_duration_purchase_binds_one_investment_and_balances_the_stock`, `test_a_mixed_line_charges_the_paid_count_only_the_stock_the_bought_counts_left`, `test_a_grant_that_moved_on_the_status_line_records_the_holding_the_line_states` |
...
| What a packed promise line was rendered from (stage-0 §257) | `application/fragment_derivation.py`: `attach_promise_derivations` writes `promise_line_derivation` into a drafting job's `prompt_sources`, in `planner.make_plan_selector`, from the one ledger read `packet_for` packs | a derivation is declared from stored columns and the pure renderer, never inferred; `valid_derivation` closes the map on any inconsistency; missing upstream identities are listed `not_recorded`; the request, the packet and the sample do not change; `build_prompt_source_view` counts derivations and shelf exposure withholds them | `test_recording_derivations_changes_no_request_byte_packet_or_sample`, `test_a_frozen_derivation_keeps_the_row_it_read_after_the_ledger_moves`, `test_shelf_exposure_withholds_every_upstream_identity`, `test_story_positions_compare_only_within_one_space_and_width`; `test_fragment_derivation.py` |
